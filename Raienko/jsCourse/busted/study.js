(function() {
    ...
    function createArticle(article) {
        var template = '<article> <h3 class="title"> ' + article.title + ' </h3>' +
            ...
            '<span class="author-name">' + article.author + '</span></div>' + '<div class="meta-data">' +
            '<span class="likes"><button data-article-id="' + article.id + '" >Likes :  ' + article.likes + '</button> </span>' +
            '<span class="data"> Date of post: ' + blog.Utils.formatDate(article.createdAt) + '</span></div>' +
            '<div class="comments"> </div>  ' +
            '</article>' +
            `<div class="add-coments-form">
                <form >
    ...
    function createComments(comment) {
        ...
        <div class="coments">
                <div class="text-holder">
                    <p class="article-text"> ${comment.comment} </p>
                </div>
                <div class="author-holder"><img src="${comment.authorPoto}" alt=""><span class="author-name">${comment.author}</span></div>
            </div>`
    }
    ...
    function renderArticles() {
        ...
        blog.Utils.sendRequest(params, function(error, data) {
            ...
            var articlesHolder = blog.Utils.createElement('articles');
    ...
    function renderArticle(id) {
        ...
        blog.Utils.sendRequest(params, function(error, data) {
            ...
            var artcielsData = [createArticle(data)];

            data.comments.forEach(function(el) {
                artcielsData.push(createComments(el))
            })

            articleHolder.innerHTML = artcielsData.join('');
    ...
    function renderAbout() {
        var aboutHolder = blog.Utils.createElement('about');
    ...
    function renderContacts() {
        var contactsHolder = blog.Utils.createElement('contacts');
