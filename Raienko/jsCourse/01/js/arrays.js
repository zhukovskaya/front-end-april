  ...
  if (isArray(array)) {
    ...
    for (i=0;i<len;i++) {
      ...
      while (!changed) {
        ...
        if (!outputArray[newPosition]) { // провер€ем зан€та ли полученна€ позици€
          outputArray[newPosition]=array[i]; 
          changed=true;
        } // если зан€та - повтор€ем цикл
      }
    }
